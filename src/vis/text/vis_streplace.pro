...
;       IDL> print, vis_streplace(s, re, expr, /evaluate, /global)
...
;       IDL> re = 'Mike([0-9]+)'
;       IDL> expr = 'fix($1) * 2'
;       IDL> help, vis_streplace('Mike5', re, expr, /evaluate)
;       <Expression>    LONG      =           10
;
...
;       IDL>   str = vis_streplace(str, re, '$1,$2', /global)
;       IDL> print, str
...
;    Michael Galloy
;-
function vis_streplace, str, pattern, replacement, $
                       evaluate=evaluate, $
                       fold_case=fold_case, $
                       global=global, $
                       start=start
  ...
  if (keyword_set(global)) then begin
    ans = vis_streplace(str, pattern, replacement, start=start, $
    ...
    while (start lt strlen(ans)) do begin
      ...
      ans =  temp $
               + vis_streplace(strmid(ans, start), pattern, replacement, $
...
; The following example demonstrates basic operations of VIS_STREPLACE, simply
; replacing "was" with "was not" in the expression "Mike was here":
print, vis_streplace('Mike was here', 'was', 'was not')

; Meta-variables $1, $2, etc. represent matched values in parentheses. This 
; swaps the first two words in the string:
print, vis_streplace('Mike was here', '([^ ]*) ([^ ]*)', '$2 $1')
...
print, vis_streplace(s, re, expr, /evaluate, /global)

; Another evaluated expression:
re = 'Mike([0-9]+)'
expr = 'fix($1) * 2'
help, vis_streplace('Mike5', re, expr, /evaluate)

; Here's an example to put grouping commas into a long integer value:
str = '1874382735872851'
re = '^[+-]?([[:digit:]]+)([[:digit:]]{3})'
for i = 0, strlen(str) / 3 - 1 do $
str = vis_streplace(str, re, '$1,$2', /global)
print, str
