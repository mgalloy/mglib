...
; 
;    Code adapted from HIST_ND from David Fanning and HIST_ND_WEIGHT by Jeremy 
;    Bailin.
;
; :Examples:
;    Try::
...
;                  0           0           0           5 
;
;    Also, see the main-level program at the end of this file which will 
...
;       when using REVERSE_INDICES with HISTOGRAM (after converting to single
...
r = 50
t = findgen(360 * 10) * !dtor
x = r * cos(t) + (randomu(seed, 360 * 10) - 0.5) * 3 + 50
y = r * sin(t) + (randomu(seed, 360 * 10) - 0.5) * 3 + 50
...
!p.multi = [0, 2, 1]
window, title='Particles', xsize=800, ysize=400, /free

plot, [0, 40], [0, 40], /nodata
plots, vis_linear_function([-1.5, 101.5], [0, 40], data=x), $
       vis_linear_function([-1.5, 101.5], [0, 40], data=y), $
       psym=!vis.psym.period

vis_image, vis_hist_nd(transpose([[x], [y]]), nbins=40), scale=10, /axes
