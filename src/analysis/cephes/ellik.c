...
double ellik(double phi,  double m) {
    ...
    a = 1.0 - m;
    if (a == 0.0) {
	if (fabs(phi) >= CEPHES_PI_2) {
	    mtherr("ellik", SING);
	    return (CEPHES_INFINITY);
	}
        /* DLMF 19.6.8, and 4.23.42 */
       return cephes_asinh(tan(phi));
    }
    npio2 = floor(phi / CEPHES_PI_2);
    if (fmod(fabs(npio2), 2.0) == 1.0)
	npio2 += 1;
    if (npio2 != 0.0) {
	K = ellpk(a);
	phi = phi - npio2 * CEPHES_PI_2;
    }
    else
	K = 0.0;
    if (phi < 0.0) {
	...
	temp = b / a;
	phi = phi + atan(t * temp) + mod * CEPHES_PI;
        denom = 1.0 - temp * t * t;
        if (fabs(denom) > 10*MACHEP) {
	    t = t * (1.0 + temp) / denom;
            mod = (phi + CEPHES_PI_2) / CEPHES_PI;
        }
        else {
            t = tan(phi);
            mod = (int)floor((phi - atan(t))/CEPHES_PI);
        }
	c = (a - b) / 2.0;
	temp = sqrt(a * b);
	a = (a + b) / 2.0;
