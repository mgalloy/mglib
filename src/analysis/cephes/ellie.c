...
double ellie(double phi, double m)
{
	...
	return (phi);
    lphi = phi;
    npio2 = floor(lphi / CEPHES_PI_2);
    if (fmod(fabs(npio2), 2.0) == 1.0)
	npio2 += 1;
    lphi = lphi - npio2 * CEPHES_PI_2;
    if (lphi < 0.0) {
	lphi = -lphi;
	sign = -1;
    }
	...
	temp = b / a;
	lphi = lphi + atan(t * temp) + mod * CEPHES_PI;
        denom = 1 - temp * t * t;
        if (fabs(denom) > 10*MACHEP) {
            t = t * (1.0 + temp) / denom;
            mod = (lphi + CEPHES_PI_2) / CEPHES_PI;
        }
        else {
            t = tan(lphi);
            mod = (int)floor((lphi - atan(t))/CEPHES_PI);
        }
	c = (a - b) / 2.0;
	temp = sqrt(a * b);
	a = (a + b) / 2.0;
