...
;       of `iterable`
;-
function mg_min, iterable, min_subscript, $
                 key_method=key_method, key_function=key_function, $
                 absolute=absolute, dimension=dimension, max=max_value, nan=nan, $
                 subscript_max=max_subscript
  compile_opt strictarr

  ; pass along normal case to MIN routine
  if (size(iterable, /type) ne 11 && n_elements(key_function) eq 0L) then begin
    return, min(iterable, min_subscript, $
                absolute=absolute, dimension=dimension, max=max, nan=nan, $
                subscript_max=max_subscript)
  endif
